...
parser.add_argument(
    ...
    type=str,
    default="work_dir/pretrain_weights/medsam_lite_best_extracted_swin.pth",
    help='path to the checkpoint of other part of MedSAM-Lite',
)
parser.add_argument(
    '-encoder_ckpt_path',
    type=str,
    default="work_dir/pretrain_weights/medsam_lite_repvit_encoder_best.pth",
...
parser.add_argument(
    ...
    default=False,
    action='store_true',
    help='whether to run the inference from the beginning'
)
...
def load_states(medsam_lite_model, repvit_ckpt_path, decoder_ckpt_path):
    repvit_ckpt = torch.load(repvit_ckpt_path)
    decoder_ckpt = torch.load(decoder_ckpt_path)
    medsam_lite_model.image_encoder.load_state_dict(repvit_ckpt["model"])
...
medsam_lite_model = load_states(medsam_lite_model, args.encoder_ckpt_path, args.lite_medsam_ckpt_path)
...
def MedSAM_infer_npz_2D(img_npz_file):
    ...
    with torch.no_grad():
        image_embedding = medsam_lite_model.image_encoder(img_256_tensor)[-1]
...
def MedSAM_infer_npz_3D(img_npz_file):
    ...
    for idx, box3D in enumerate(boxes_3D, start=1):
        ...
        for z in range(z_middle, z_max+1):
            ...
            with torch.no_grad():
                image_embedding = medsam_lite_model.image_encoder(img_256_tensor)[-1]  # (1, 256, 64, 64)
            if z == z_middle:
...
if __name__ == '__main__':
    ...
    for img_npz_file in tqdm(img_npz_files):
        if overwrite:
            if basename(img_npz_file) in infered_npz_files:
                continue
        start_time = time()
